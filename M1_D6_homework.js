...
const howManyDays = function (receivedDate) {
    ...
    console.log(`${ daysPassed } days have passed since the specified date.`)
}
...
console.log(deleteProp(user1, 'age'))
...
const oldestMovie = function () {
    let oldestMovie = {}
    let oldestMovieYear = parseInt(movies[0].Year)
    for (let i = 0; i < movies.length; i++) {
        if (parseInt(movies[i].Year) < oldestMovieYear) {
            oldestMovie = movies[i]
        }
    }
    console.log(`"${ oldestMovie.Title }" is the oldest movie in the array.`)
}
...
const countMovies = function () {
    console.log(`There are ${ movies.length } movies in the provided array.`)
}
...
const onlyInThisMillennium = function () {
    let thisMilleniumMovies = []
    for (let i = 0; i < movies.length; i++) {
        if (parseInt(movies[i].Year) >= 2000) {
            thisMilleniumMovies.push(movies[i])
        }
    }
    return thisMilleniumMovies
}
...
const getMovieById = function (receivedId) {
    let queriedMovie = {}
    for (let i = 0; i < movies.length; i++) {
        if (receivedId === movies[i].imdbID) {
            queriedMovie = movies[i]
        }
    }
    return queriedMovie
}
...
const searchAndDivide = function (receivedStr) {
    let objToReturn = {
        match: [],
        umatch: []
    }
    for (let i = 0; i < movies.length; i++) {
        if (movies[i].Title.toLowerCase().includes(receivedStr.toLowerCase())) {
            objToReturn.match.push(movies[i])
        } else {
            objToReturn.umatch.push(movies[i])
        }
    }
    return objToReturn
}
